...
type D3ForceLink = {
  distance?: (
    d: number | ((link: RFLink) => number)
  ) => D3ForceLink | undefined;
  strength?: (s: number) => D3ForceLink | undefined;
};
type D3ForceManyBody = {
  strength?: (s: number) => D3ForceManyBody | undefined;
};
...
function getNodeUrl(n: unknown): string | undefined {
  ...
  const obj = n as Partial<
    Record<'url' | 'profileUrl' | 'linkedinUrl', unknown>
  >;
  const v =
...
const GraphCanvas: React.FC<Props> = ({
  ...
  const graph = useMemo(() => {
    ...
    const normalizedLinks: RFLink[] = (rawLinks as ReadonlyArray<LinkEdge>).map(
      (l) => ({
        source: l.source ?? '',
        target: l.target ?? '',
