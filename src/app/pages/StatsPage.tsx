...
function topCounts(rows: LinkedInRawRecord[], field: 'company' | 'title', topN = 10): CountItem[] {
...
export default function StatsPage() {
  ...
  const filtered = useMemo(() => {
    return raw.filter((r: LinkedInRawRecord) => {
      ...
      const inRange =
        (!fromDate || (d && d >= new Date(fromDate))) && (!toDate || (d && d <= new Date(toDate)));
  ...
  if (raw.length === 0) {
    return <p className="text-gray-600">No data yet. Go to Import and upload a CSV.</p>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <section className="mt-8 bg-white border rounded-lg p-4">
        <h3 className="font-semibold mb-3">Connections by Month (last 24 months)</h3>
          ...
          {kwEntries.map(([k, v]) => {
            ...
            return (
              <button
                ...
                aria-pressed={on}
              >
                {k} <span className={on ? 'opacity-90' : 'text-gray-500'}>({v})</span>
              </button>
            );
          })}
        </div>
      ...
      <section className="mt-8 bg-white border rounded-lg p-4">
        ...
        <ul className="divide-y">
          ...
          {recent.length === 0 && <li className="py-2 text-sm text-gray-500">No recent data</li>}
        </ul>
      </section>
    </div>
  );
...
function TopList({ items, empty }: { items: CountItem[]; empty: string }) {
  if (items.length === 0) return <div className="text-sm text-gray-500">{empty}</div>;
  return (
