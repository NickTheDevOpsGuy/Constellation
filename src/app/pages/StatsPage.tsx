...
export default function StatsPage() {
  ...
  const months = useMemo<MonthPoint[]>(() => monthSeries(filtered, 24), [filtered]);
    ...
    return (
      <div className="min-h-[calc(100vh-56px)] bg-gradient-to-b from-[#0b1220] via-[#0f1a2d] to-[#1a1f3a] flex items-center justify-center px-4">
        <p className="text-slate-400">
          No data yet. Go to <span className="font-medium text-slate-200">Import</span> and upload a
          CSV.
        </p>
      </div>
    );
    ...
    <div className="max-w-6xl mx-auto pt-8 sm:pt-10">
          ...
          {kwEntries.map(([k, v]) => {
            ...
            return (
              <button
                ...
                aria-pressed={on}
              >
                {k} <span className={on ? 'opacity-90' : 'text-slate-400'}>({v})</span>
              </button>
            );
          })}
        </div>
      ...
      <section className="mt-8 rounded-xl border border-white/10 bg-slate-950/50 backdrop-blur p-4 mb-8">
        ...
        <ul className="divide-y divide-white/10">
          ...
          {recent.length === 0 && <li className="py-2 text-sm text-slate-400">No recent data</li>}
        </ul>
      </section>
    </div>
  );
