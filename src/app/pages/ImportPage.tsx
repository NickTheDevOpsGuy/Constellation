  ...
  async function handleFile(file: File) {
    ...
    if (name.endsWith('.zip')) {
      ...
      if (got.comments || got.reactions) {
        await loadInteractions({ commentsCsv: got.comments ?? null, reactionsCsv: got.reactions ?? null });
      }
      ...
      <div className="rounded-2xl border border-white/10 bg-slate-950/60 backdrop-blur-md shadow-xl p-6">
        <header className="mb-6">
          ...
          <p className="mt-1 text-sm text-slate-300">
            Drop your LinkedIn export ZIP or paste the Connections CSV. We’ll parse connections, posts,
            comments, reactions, and invitations when present.
          </p>
        </header>
